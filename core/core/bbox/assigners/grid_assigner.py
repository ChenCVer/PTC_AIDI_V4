...
class GridAssigner(BaseAssigner):
    ...
    def assign(self, bboxes, box_responsible_flags, gt_bboxes, gt_labels=None):
        ...
        1. 对所有anchor标记为-1, 表示为忽略样本;
        2. 如果anchor与所有的gt的iou均小于net_iou_thr, 标记为负样本, 标记为0.
        3. 对于每一个cell里面的每一个anchor, 计算与所有的gt的iou, 如果IOU>post_iou,
           且该anchor所在网格有负责预测的gt, 则该anchor为正样本,
        4. 对每一个gt, 基于iou最大原则, 找到其对应的cell中的anchor作为正样本,
           主要是为了保证每个gt都有对应的anchor.
        1. assign every anchor to -1,
        2. assign proposals whose iou with all gts <= neg_iou_thr to 0, 负样本.
        3. for each anchor within a cell, if the iou with its nearest gt >
            pos_iou_thr and the center of that gt falls inside the cell,
            assign it to that anchor,
        4. for each gt, assign its nearest proposals within the cell the
            gt falls in to itself.
        ...
        """
        # 三个层的所有anchor和gt, 注意, 所有anchor都是原尺度, 没有进行对应的下采样操作
        num_gts, num_bboxes = gt_bboxes.size(0), bboxes.size(0)
        # 这里overlap的计算为所有gt和所有anchor(三个尺度上)都是在输入图像尺度上进行.
        # compute iou between all gt and bboxes, overlaps.shape=[num_gts, num_anchors]
        ...
        # 对于每个anchor, 找到和他最匹配的gt,基于iou最大原则, 这里是anchor和gt之间算iou
        ...
        # 对于每个anchor，找到和他最匹配的gt, 基于iou最大原则
        max_overlaps, argmax_overlaps = overlaps.max(dim=0)

        # for each gt, which anchor best overlaps with it
        # for each gt, the max iou of all proposals
        # shape of gt_max_overlaps == gt_argmax_overlaps == num_gts
        # TODO: 对于每个gt，找到和他最匹配的anchor, 这里由于gt同时和三个尺度的anchor在原图尺度做匹配
        ...
        # 这里,可能有些gt bbox还没有anchor匹配,后面会解决, 这里会出现gt跨层预测的现象.
        pos_inds = (max_overlaps > self.pos_iou_thr) & box_responsible_flags.type(torch.bool)
        # 正样本anchor，保存对应的gt bbox索引，表示和其匹配上
        assigned_gt_inds[pos_inds] = argmax_overlaps[pos_inds] + 1

        # 4. assign positive to max overlapped anchors within responsible cell
        # 遍历每个Gt,判断和他在同一个cell中的iou最大的anchor, 是否大于self.min_pos_iou=0阈值
        # 是的话也定义为正样本, 一般设置为0, 可以保证每个gt一定有anchor进行匹配, 但是这降低了正样
        # 本质量, 这个值应该考虑设置自适应, 前期为了加速收敛, 可以设置为0, 后期可以提高该值, 从而
        # 提高正样本质量, 减少网络训练中出现loss震荡.
        for i in range(num_gts):
            if gt_max_overlaps[i] > self.min_pos_iou:
                if self.gt_max_assign_all:
                    # 找出anchor索引, 一定要满足box_responsible_flags[...]=True才行
