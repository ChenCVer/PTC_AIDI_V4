...
class SEBottleneck(_SEBottleneck):
    ...
    def __init__(self,
                 ...
                 se_ratio=16,
                 **kwargs):
        super(SEBottleneck, self).__init__(inplanes, planes, se_ratio, **kwargs)
        self.groups = groups
        ...
        if groups != 1:
            assert self.planes % base_channels == 0
            self.planes = (
                groups * width_per_group * self.planes // base_channels)

        self.norm1_name, norm1 = build_norm_layer(
            self.norm_cfg, self.planes, postfix=1)
        self.norm2_name, norm2 = build_norm_layer(
            self.norm_cfg, self.planes, postfix=2)
        self.norm3_name, norm3 = build_norm_layer(
            self.norm_cfg, self.planes * 2, postfix=3)
