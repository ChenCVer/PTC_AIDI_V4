...
from core.ops import sigmoid_focal_loss as _sigmoid_focal_loss
...
def softmax_focal_loss(pred,
    ...
    if isinstance(alpha, (float, int)): alpha = torch.Tensor([alpha] * num_classes)
    if isinstance(alpha, list): alpha = torch.Tensor(alpha)

    target = target.view(-1, 1)
    prob = F.softmax(pred, dim=1)
    prob = prob.gather(1, target).view(-1, 1)

    if alpha is not None:
        if alpha.type() != pred.data.type():
            alpha = alpha.type_as(pred.data)
        alpha = alpha.gather(0, target.data.view(-1)).view(-1, 1)

    # loss = - α x [(1 - p) ** γ] * log(p)
    loss = -alpha * (torch.pow((1 - prob), gamma)) * torch.log(prob)
    # weighted
    loss = weight_reduce_loss(loss, weight, reduction, avg_factor)
