        ...
        if i != N - 1:
            ...
            if tscore < maxscore:
                dets[i], dets[maxpos.item() + i + 1] = \
                    dets[maxpos.item() + i + 1].clone(), dets[i].clone()
                scores[i], scores[maxpos.item() + i + 1] = \
                    scores[maxpos.item() + i + 1].clone(), scores[i].clone()
                areas[i], areas[maxpos + i + 1] = \
                    areas[maxpos + i + 1].clone(), areas[i].clone()
...
def box_iou(box1, box2, xyxy=True):
    ...
    if xyxy:
        ...
        # Intersection area
        tl = torch.max((box1[:, None, :2] - box1[:, None, 2:] / 2),
                       (box2[:, :2] - box2[:, 2:] / 2))  # x1,y1
        # # intersection bottom right
        br = torch.min((box1[:, None, :2] + box1[:, None, 2:] / 2),
                       (box2[:, :2] + box2[:, 2:] / 2))  # x2,y2
