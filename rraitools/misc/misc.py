...
def calc_iter_every_epoch(dataloader, batch_size):
    """
    TODO: 本函数还需要考虑在线训练和离线训练兼容的问题.
    :param dataloader:
    :param batch_size:
    :return:
    注: len(dataloader) == len(dataloader.dataset) // batchsize
    """
    if len(dataloader) * batch_size != len(dataloader.dataset):
        # 离线训练模式
        iter_pre_epoch = len(dataloader.dataset) // batch_size
    else:
        # 在线训练模式
        iter_pre_epoch = len(dataloader.dataset)

    return iter_pre_epoch