...
class LrUpdaterHook(Hook):
    ...
    def __init__(self,
                 by_epoch=True,  # true表示step参数是以epoch为单位，否则以iter为单位
                 warmup=None,
                 warmup_iters=0,
                 warmup_ratio=0.1,
                 print_interval_iter=None,
                 **kwargs):
    ...
    def before_run(self, runner):
        ...
        self.base_lr = [group['initial_lr'] for group in runner.optimizer.param_groups]  # 设置初始学习率
    ...
    def before_train_iter(self, runner):
        ...
        cur_iter = runner.total_train_iter
        if not self.by_epoch:  # 通过epoch, 进行学习率衰减.
            self.regular_lr = self.get_regular_lr(runner)
            ...
            if self.warmup is None or cur_iter > self.warmup_iters:
                if runner.inner_iter % self._print_interval_iter == 0:
                    runner.logger.debug('lr: %.7f' % runner.optimizer.param_groups[0]['lr'])
                return
