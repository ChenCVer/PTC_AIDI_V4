...
class TextLoggerHook(LoggerHook):
    ...
    def _log_info(self, log_dict, runner):
        ...
        if runner.mode == 'train':
            ...
            if self.by_epoch:
                log_str = f'Epoch [{log_dict["epoch"]}][{log_dict["iter"]}/{len(runner.data_loader)}]({runner.mode})\t'
            else:
            ...
            if self.by_epoch:
                log_str = f'Epoch [{log_dict["epoch"]}][{log_dict["iter"]}]({runner.mode})\t'
            else:
                log_str = f'Iter({runner.mode}) [{log_dict["iter"]}]\t'
    ...
    def log(self, runner):
        ...
        if runner.mode == 'train':
            ...
            # statistic memory
            if torch.cuda.is_available():
                log_dict['memory'] = self._get_max_memory(runner)  # 获取的是最大内存

        for name, val in runner.log_buffer.output.items():
            if name in ['time', 'data_time', 'prediction']:
                continue
            log_dict[name] = val
