...
def pytorch2onnx(config_path,
    ...
    if verify:
        ...
        # todo: 2021-02-01: 由于分类的get_results和seg, det没做统一, 这里onnx_results是tensor输出
        if isinstance(pytorch_results, torch.Tensor):
            pytorch_results = pytorch_results.detach().cpu().numpy()
            onnx_results = onnx_results.detach().cpu().numpy()
...
def parse_args():
    ...
    parser.add_argument('--checkpoint',
                        default="convert_model/res2net.pth",
                        help='checkpoint file')
    parser.add_argument('--input-img', type=str,
                        default="convert_model/pullution.bmp",
                        help='Images for input')
    parser.add_argument('--show',
                        type=bool,
                        default=True,
                        help='show onnx graph')
    parser.add_argument('--output-file', type=str, default='convert_model/res2net.onnx')
